...
def test_register_kwargs_decorator_calls_event(mocker, mock_model) -> None:
    ...
    mock_record_event = mocker.patch("src.dry_torch.events.RecordMetadata",
                                     return_value=TestEvent())

    @register_kwargs
    def sample_init(instance, model, /, **kwargs):
        _not_used = instance, model, kwargs

    mock_instance = mocker.MagicMock()
    sample_init(mock_instance, mock_model, arg1=10, name="TestName")
...
def test_register_kwargs_raises_model_first_error(mocker) -> None:
    ...
    with pytest.raises(exceptions.ModelFirstError):
        sample_init(mock_instance, "not_a_model", arg1=10)  # type: ignore
...
def test_register_kwargs_warns_for_non_keyword_args(mocker, mock_model) -> None:
    ...
    @register_kwargs
    def sample_init(instance, model, arg, **kwargs):
        _not_used = instance, model, arg, kwargs

    mock_instance = mocker.MagicMock()
    with pytest.warns(exceptions.NotDocumentedArgs):
        sample_init(mock_instance, mock_model, 'pos_arg')
...
def test_register_kwargs_sets_default_name(mocker, mock_model) -> None:
    ...
    @register_kwargs
    def sample_init(instance, model, /, name='', **kwargs):
        _not_used = instance, model, kwargs
        return name

    mock_instance = mocker.MagicMock()
    default_name = sample_init(mock_instance, mock_model, arg1=10)
